    ...
    while True:
        ...
        match opcao:
            ...
            case '5':
                try:
                    print(f'Arquivo aberto: {abrir_arquivo}.json \n')
                    codigo = int(input('Informe o código do usuario que deseja deletar: '))
                    nome_deletado = usuarios[codigo]['nome']
                    confirmacao = input(f"Deseja deletar o usuario: {nome_deletado}? Digite 'SIM' para confirmar. ").upper()
                    if confirmacao == 'SIM':
                        del(usuarios[codigo])
                        print(m.salvar_dados(usuarios, abrir_arquivo))
